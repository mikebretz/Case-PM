...
def deploy_accounting_check(app_root: str | None = None) -> dict:
    ...
    try:
        ...
        mirror = sage_mirror_deploy_check_v9()
        results.append({'check': 'sage_mirror_v9', 'ok': mirror.get('ok'), 'detail': str(mirror)[:600]})
    except Exception as exc:
        results.append({'check': 'sage_mirror_v9', 'ok': False, 'detail': str(exc)})
    try:
        from accounting_waves_37 import sage_mirror_deploy_check_v10
        mirror = sage_mirror_deploy_check_v10()
        results.append({'check': 'sage_mirror_v10', 'ok': mirror.get('ok'), 'detail': str(mirror)[:600]})
    except Exception as exc:
        results.append({'check': 'sage_mirror_v10', 'ok': False, 'detail': str(exc)})
    try:
        from accounting_waves_38 import sage_mirror_deploy_check_v11
        mirror = sage_mirror_deploy_check_v11()
        results.append({'check': 'sage_mirror_v11', 'ok': mirror.get('ok'), 'detail': str(mirror)[:600]})
    except Exception as exc:
        results.append({'check': 'sage_mirror_v11', 'ok': False, 'detail': str(exc)})
    try:
        from accounting_waves_42 import sage_mirror_deploy_check_v12
        mirror = sage_mirror_deploy_check_v12()
