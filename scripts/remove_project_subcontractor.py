...
def main() -> int:
    ...
    parser.add_argument('--company', help='Subcontractor company name (partial match)')
    parser.add_argument('--company-id', help='Subcontractor company id key (e.g. 9)')
    ...
    if not args.company and not args.company_id:
        print('Provide --company and/or --company-id')
        return 1
    ...
    with app_module.app.app_context():
        ...
        for c in commitments:
            if args.company_id and str(c.company_id or '').strip() == str(args.company_id).strip():
                matches.append(c)
            elif args.company and args.company.lower() in (c.company_name or '').lower():
                matches.append(c)
        if not matches:
            label = args.company or args.company_id
            print(f'No commitments matching "{label}"')
        else:
            for c in matches:
        ...
        if args.company_id and str(args.company_id) not in sov_keys:
            sov_keys.append(str(args.company_id))
        print(f'SOV keys to purge: {sov_keys or "(none found by name — will match by company id/name)"}')
        ...
        if matches:
            company_id = company_id or matches[0].company_id
