...
def _run_estimating_bid(rt: ProjectRuntime, models: dict) -> None:
    ...
    rt.result.metrics['estimate_total'] = float(est.total_amount or 0)
    if abs(float(est.total_amount or 0) - cv) > 250_000:
        rt.result.add(
            'warning',
            'estimating',
            f'Estimate total ${est.total_amount:,.0f} vs contract ${cv:,.0f}',
        )
...
def _closeout(rt: ProjectRuntime, models: dict) -> None:
    ...
    rfi_open = RFI.query.filter(
        RFI.project_id == project.id,
        RFI.status.notin_(('Closed', 'Void')),
    ).count()
    co_pending = ChangeOrder.query.filter(
        ChangeOrder.project_id == project.id,
        ChangeOrder.status.notin_(('Approved', 'Rejected', 'Void')),
    ).count()
    rt.result.metrics['closeout_open_rfis'] = rfi_open
    rt.result.metrics['closeout_pending_cos'] = co_pending
    if rfi_open > 25:
        rt.result.add('warning', 'closeout', f'{rfi_open} RFIs still open at substantial completion')
    if co_pending > 15:
        rt.result.add('warning', 'closeout', f'{co_pending} change orders still pending at closeout')

    if Submittal:
        sub_open = Submittal.query.filter(
            Submittal.project_id == project.id,
            Submittal.status.notin_(('Closed', 'Void')),
        ).count()
        rt.result.metrics['closeout_open_submittals'] = sub_open
        if sub_open > 40:
            rt.result.add('info', 'closeout', f'{sub_open} submittals still open (typical on large jobs)')
    ...
    lines = (budget or {}).get('budgetLines') or []
    approved_chg = sum(float(l.get('approved_changes') or 0) for l in lines)
    rt.result.metrics['budget_approved_changes'] = approved_chg
    if approved_chg and abs(approved_chg - TARGET_CO_VALUE) > 750_000:
        rt.result.add(
            'warning',
            'accounting',
            f'Budget approved_changes ${approved_chg:,.0f} vs ~${TARGET_CO_VALUE:,.0f} CO target',
        )
...
def run_hundred_million_lifecycle(*, seed: int = 100) -> ProjectRuntime:
    ...
    try:
        with app_module.app.app_context():
            ...
            sub_user = rt.users['pm']
            orig_sub = rt.users.get('sub')
            if orig_sub:
                rt.users['sub'] = sub_user
            for gm in range(scenario.duration_months):
                ...
                db.session.commit()
            if orig_sub:
                rt.users['sub'] = orig_sub
            rt.result.add(
                'warning',
                'submittals',
                'Sim subcontractor user lacks submittals module view — submittal workflow used PM actor (permission model gap)',
            )
