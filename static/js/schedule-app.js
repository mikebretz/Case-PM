...
(function () {
    ...
    function layoutColumnResizeGrips(metrics) {
        ...
        if (trailing) {
            let edge = 0;
            metrics.forEach(m => {
                if (m.col?.resize === false || isAddColumnCol(m.col)) return;
                edge = m.left + m.width;
            });
    ...
    function ensureColumnResizeGrips() {
        ...
        const place = () => {
            ...
            if (m) grip.style.left = Math.max(0, m.left + m.width - 5) + 'px';
        };
        place();
