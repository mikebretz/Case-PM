...
(function (global) {
  ...
  async function render() {
    ...
    const [tree, runs, ctb] = await Promise.all([
        ...
        <div>
          ...
          <div class="border border-zinc-700 rounded-lg divide-y divide-zinc-800 text-xs max-h-40 overflow-y-auto">
            ${(runs.runs || []).map((r) => `<div class="p-2 flex justify-between items-center">
              ...
              ${r.status === 'Open' ? `<button type="button" class="acct-con-elim text-emerald-400 bg-transparent border-none cursor-pointer" data-id="${r.id}">Post eliminations</button>` : `<span class="text-zinc-600">JE ${esc(r.elimination_batch_id || '—')}</span>`}
