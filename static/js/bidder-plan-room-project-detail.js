(function () {
  ...
  function meetingLine(label, block) {
    if (!block || (!block.date && !block.notes)) return '';
    return `<p class="pr-project-meta"><strong>${esc(label)}:</strong> ${formatDate(block.date)}${block.time ? ` ${esc(block.time)}` : ''}${block.location ? ` · ${esc(block.location)}` : ''}${block.mandatory ? ' (mandatory)' : ''}</p>`;
  }
    ...
    .then((data) => {
      ...
      const docsPanel = sections.length
        ...
        : (docs.length ? `<ul class="pr-doc-list">${docs.map((d) => `
            ...
            <a class="pr-download" href="${esc(d.download_url)}">Download</a></li>`).join('')}</ul>` : '<p class="pr-muted">No documents attached yet.</p>');
      ...
      host.innerHTML = `
        <div class="pr-detail-hero">
          ...
          ${pr.instructions_html ? `<div class="pr-html-content" style="margin-top:0.75rem">${pr.instructions_html}</div>` : ''}
        ...
        <div id="prPanelPackages" class="pr-panel hidden">
          ${pkgs.length ? pkgs.map((pkg) => `
            <div class="pr-pkg-card">
              ...
              <a class="pr-download" href="${esc(pkg.detail_url || '#')}">View full ITB &amp; documents →</a>
            </div>
          `).join('') : '<p class="pr-muted">No published packages.</p>'}
        </div>
        <div id="prPanelAddenda" class="pr-panel hidden">
          ${addenda.length ? addenda.map((a) => `
            <div class="pr-pkg-card">
      ...
      host.querySelectorAll('.pr-subnav-tabs button').forEach((btn) => {
        btn.addEventListener('click', () => {
          host.querySelectorAll('.pr-subnav-tabs button').forEach((b) => b.classList.remove('active'));
