...
def register_accounting_routes(app, deps):
    ...
    def api_acct_sage_push_ap_live():
        ...
        out = sage_push_open_ap_idempotent(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})
    ...
    def api_acct_retainage_release(project_id):
        ...
        try:
            ...
            try:
                ...
                out['sage'] = push_retainage_release_to_sage(db, models, _ledger_id(), out['ar_document_id'], user_id=current_user.id)
            except Exception as exc:
                out['sage'] = {'skipped': str(exc)}
            db.session.commit()
            return jsonify({'ok': True, **out})
        except Exception as exc:
            db.session.rollback()
    ...
    # --- Waves 20–24 Sage mirror depth ---

    @app.route('/api/accounting/sage/sync/push-ar-receipts', methods=['POST'])
    @login_required
    def api_acct_sage_push_ar_receipts():
        from accounting_waves_25 import sage_push_ar_receipt_batches
        try:
            out = sage_push_ar_receipt_batches(db, models, _ledger_id(), user_id=current_user.id)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except PermissionError as exc:
            db.session.rollback()
    ...
    @app.route('/api/accounting/sage/sync/push-open-ar-idempotent', methods=['POST'])
    @login_required
    def api_acct_sage_push_ar_idempotent():
        from accounting_waves_25 import sage_push_open_ar_idempotent
        try:
            out = sage_push_open_ar_idempotent(db, models, _ledger_id(), user_id=current_user.id)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except PermissionError as exc:
            db.session.rollback()
    ...
    @app.route('/api/accounting/sage/segment-map', methods=['GET', 'POST'])
    @login_required
    def api_acct_sage_segment_map():
        from accounting_waves_25 import save_gl_segment_map
        ledger = models['AcctLedger'].query.get(_ledger_id())
        from accounting_waves_24 import _ledger_settings
        if request.method == 'GET':
            return jsonify({'segment_map': (_ledger_settings(ledger).get('sage_gl_segment_map') or {})})
        out = save_gl_segment_map(db, models, _ledger_id(), request.get_json(silent=True) or {}, user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/distribution/push-po', methods=['POST'])
    @login_required
    def api_acct_sage_push_po():
        from accounting_waves_25 import sage_push_po_receipts_live
        out = sage_push_po_receipts_live(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/distribution/push-ic', methods=['POST'])
    @login_required
    def api_acct_sage_push_ic():
        from accounting_waves_25 import sage_push_ic_transactions
        out = sage_push_ic_transactions(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/distribution/push-oe', methods=['POST'])
    @login_required
    def api_acct_sage_push_oe():
        from accounting_waves_25 import sage_push_oe_shipments
        out = sage_push_oe_shipments(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/distribution/pull-ic', methods=['POST'])
    @login_required
    def api_acct_sage_pull_ic():
        from accounting_waves_25 import sage_pull_ic_snapshot
        out = sage_pull_ic_snapshot(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/distribution/exceptions', methods=['GET'])
    @login_required
    def api_acct_sage_dist_exceptions():
        from accounting_waves_25 import sage_distribution_exception_inbox
        return jsonify(sage_distribution_exception_inbox(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/portfolio/dashboard', methods=['GET'])
    @login_required
    def api_acct_sage_portfolio():
        from accounting_waves_25 import sage_portfolio_ops_dashboard
        return jsonify(sage_portfolio_ops_dashboard(db, models, _ledger_id(), Project=Project))

    @app.route('/api/accounting/jobcost/<int:project_id>/sage-resolve', methods=['POST'])
    @login_required
    def api_acct_sage_job_resolve(project_id):
        from accounting_waves_25 import sage_resolve_job_actual_conflicts
        data = request.get_json(silent=True) or {}
        out = sage_resolve_job_actual_conflicts(
            db, models, _ledger_id(), project_id,
            strategy=(data.get('strategy') or 'casepm'),
            user_id=current_user.id, Project=Project,
        )
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/platform/fx-rates', methods=['POST'])
    @login_required
    def api_acct_sage_fx():
        from accounting_waves_25 import sage_sync_fx_rates
        out = sage_sync_fx_rates(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/platform/consolidation-export', methods=['POST'])
    @login_required
    def api_acct_sage_consolidation():
        from accounting_waves_25 import sage_export_consolidation_eliminations
        try:
            out = sage_export_consolidation_eliminations(db, models, _ledger_id(), user_id=current_user.id)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except Exception as exc:
            db.session.rollback()
    ...
    @app.route('/api/accounting/sage/platform/fa-queue', methods=['POST'])
    @login_required
    def api_acct_sage_fa_queue():
        from accounting_waves_25 import sage_queue_fa_depreciation
        out = sage_queue_fa_depreciation(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/payroll/sor-policy', methods=['GET', 'POST'])
    @login_required
    def api_acct_sage_payroll_sor():
        from accounting_waves_25 import save_payroll_sor_policy
        from accounting_waves_24 import _ledger_settings
        ledger = models['AcctLedger'].query.get(_ledger_id())
        if request.method == 'GET':
            s = _ledger_settings(ledger)
            return jsonify({
                'payroll_system_of_record': s.get('payroll_system_of_record') or 'casepm',
                'sage_pr_push_enabled': s.get('sage_pr_push_enabled') == '1',
            })
        out = save_payroll_sor_policy(db, models, _ledger_id(), request.get_json(silent=True) or {}, user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/compliance/1099-reconcile', methods=['GET'])
    @login_required
    def api_acct_sage_1099_reconcile():
        from accounting_waves_25 import sage_1099_vendor_reconcile
        yr = request.args.get('tax_year', type=int) or date.today().year
        return jsonify(sage_1099_vendor_reconcile(db, models, _ledger_id(), yr))

    @app.route('/api/accounting/sage/compliance/year-end-variance', methods=['GET'])
    @login_required
    def api_acct_sage_year_end_var():
        from accounting_waves_25 import sage_year_end_variance_report
        yr = request.args.get('tax_year', type=int) or date.today().year
        return jsonify(sage_year_end_variance_report(db, models, _ledger_id(), yr))

    @app.route('/api/accounting/cron/sage-mirror-full', methods=['POST'])
    def api_acct_cron_sage_mirror_full():
        from accounting_waves_25 import cron_waves_20_24_maintenance
        secret = request.headers.get('X-CasePM-Cron-Secret') or (request.get_json(silent=True) or {}).get('secret', '')
        try:
            out = cron_waves_20_24_maintenance(db, models, secret, Project=Project)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except PermissionError as exc:
            return jsonify({'error': str(exc)}), 403

    # --- Waves 25–28 Sage mirror completion ---

    @app.route('/api/accounting/sage/sync/pull-ar-receipts', methods=['POST'])
    @login_required
    def api_acct_sage_pull_ar_receipts():
        from accounting_waves_26 import sage_pull_ar_receipt_applications
        try:
            out = sage_pull_ar_receipt_applications(db, models, _ledger_id(), user_id=current_user.id)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except PermissionError as exc:
            db.session.rollback()
    ...
    @app.route('/api/accounting/sage/sync/pull-ap-status', methods=['POST'])
    @login_required
    def api_acct_sage_pull_ap_status():
        from accounting_waves_26 import sage_pull_ap_payment_status
        try:
            out = sage_pull_ap_payment_status(db, models, _ledger_id(), user_id=current_user.id)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except PermissionError as exc:
            db.session.rollback()
    ...
    @app.route('/api/accounting/sage/distribution/reconcile', methods=['GET'])
    @login_required
    def api_acct_sage_ic_oe_reconcile():
        from accounting_waves_26 import sage_pull_oe_and_reconcile
        out = sage_pull_oe_and_reconcile(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify(out)

    @app.route('/api/accounting/sage/segment-map/validate', methods=['GET'])
    @login_required
    def api_acct_sage_segment_validate():
        from accounting_waves_26 import validate_segment_map_preflight
        return jsonify(validate_segment_map_preflight(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/construction/void', methods=['POST'])
    @login_required
    def api_acct_sage_construction_void():
        from accounting_waves_26 import sage_void_construction_reversal
        data = request.get_json(silent=True) or {}
        key = (data.get('source_key') or '').strip()
        if not key:
            return jsonify({'error': 'source_key required'}), 400
        try:
            out = sage_void_construction_reversal(db, models, _ledger_id(), key, user_id=current_user.id)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except Exception as exc:
            db.session.rollback()
    ...
    @app.route('/api/accounting/sage/sync/retry-inbox', methods=['POST'])
    @login_required
    def api_acct_sage_retry_inbox():
        from accounting_waves_26 import sage_inbox_auto_retry
        out = sage_inbox_auto_retry(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/platform/company-code', methods=['GET'])
    @login_required
    def api_acct_sage_company_code():
        from accounting_waves_26 import resolve_transaction_company_code
        pid = request.args.get('project_id', type=int)
        return jsonify({'company_code': resolve_transaction_company_code(db, models, _ledger_id(), pid, Project=Project)})

    @app.route('/api/accounting/sage/platform/consolidation-roundtrip', methods=['POST'])
    @login_required
    def api_acct_sage_consolidation_rt():
        from accounting_waves_26 import sage_consolidation_round_trip
        out = sage_consolidation_round_trip(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/platform/gl-security', methods=['POST'])
    @login_required
    def api_acct_sage_gl_security():
        from accounting_waves_26 import sage_sync_gl_security_groups
        out = sage_sync_gl_security_groups(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/platform/fx-revaluation', methods=['POST'])
    @login_required
    def api_acct_sage_fx_reval():
        from accounting_waves_26 import sage_push_fx_revaluation
        out = sage_push_fx_revaluation(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/platform/fa-push', methods=['POST'])
    @login_required
    def api_acct_sage_fa_push():
        from accounting_waves_26 import sage_push_fa_assets_live
        out = sage_push_fa_assets_live(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})
    ...
    @app.route('/api/accounting/sage/compliance/1099-tieout', methods=['GET'])
    @login_required
    def api_acct_sage_1099_tieout():
        from accounting_waves_26 import sage_1099_dollar_tieout
        yr = request.args.get('tax_year', type=int) or date.today().year
        return jsonify(sage_1099_dollar_tieout(db, models, _ledger_id(), yr))

    @app.route('/api/accounting/sage/compliance/apply-tax-groups', methods=['POST'])
    @login_required
    def api_acct_sage_apply_tax():
        from accounting_waves_26 import apply_tax_groups_to_open_documents
        out = apply_tax_groups_to_open_documents(db, models, _ledger_id(), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/compliance/year-end-extended', methods=['GET'])
    @login_required
    def api_acct_sage_year_end_ext():
        from accounting_waves_26 import sage_year_end_extended_report
        yr = request.args.get('tax_year', type=int) or date.today().year
        return jsonify(sage_year_end_extended_report(db, models, _ledger_id(), yr))

    @app.route('/api/accounting/cron/sage-mirror-complete', methods=['POST'])
    def api_acct_cron_sage_mirror_complete():
        from accounting_waves_26 import cron_waves_25_28_maintenance
        secret = request.headers.get('X-CasePM-Cron-Secret') or (request.get_json(silent=True) or {}).get('secret', '')
        try:
            out = cron_waves_25_28_maintenance(db, models, secret, Project=Project)
            db.session.commit()
            return jsonify({'ok': True, **out})
        except PermissionError as exc:
            return jsonify({'error': str(exc)}), 403
    ...
    @app.route('/api/accounting/sage/profile-packs', methods=['GET'])
    @login_required
    def api_acct_sage_profile_packs():
        from accounting_waves_28 import list_sage_profile_packs
        return jsonify(list_sage_profile_packs())

    @app.route('/api/accounting/sage/profile-packs/apply', methods=['POST'])
    @login_required
    def api_acct_sage_profile_pack_apply():
        from accounting_waves_28 import apply_profile_pack
        data = request.get_json(silent=True) or {}
        pack_id = (data.get('pack_id') or 'sage300_cre_2024').strip()
        out = apply_profile_pack(db, models, _ledger_id(), pack_id, user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/sage/push-dry-run', methods=['POST'])
    @login_required
    def api_acct_sage_dry_run():
        from accounting_waves_28 import sage_push_dry_run
        data = request.get_json(silent=True) or {}
        out = sage_push_dry_run(db, models, _ledger_id(), data.get('document_type', 'ar'), int(data.get('document_id') or 0))
        return jsonify(out)

    @app.route('/api/accounting/sage/sync-health', methods=['GET'])
    @login_required
    def api_acct_sage_sync_health():
        from accounting_waves_28 import sage_sync_health_score
        return jsonify(sage_sync_health_score(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/runbook', methods=['GET'])
    @login_required
    def api_acct_sage_runbook():
        from accounting_waves_28 import sage_admin_runbook
        return jsonify(sage_admin_runbook(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/drift/panel', methods=['GET'])
    @login_required
    def api_acct_sage_drift_panel():
        from accounting_waves_28 import sage_drift_panel_payload
        return jsonify(sage_drift_panel_payload(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/merge/compare', methods=['GET'])
    @login_required
    def api_acct_sage_merge_compare():
        from accounting_waves_28 import sage_merge_compare
        return jsonify(sage_merge_compare(
            db, models, _ledger_id(),
            request.args.get('entity_type', 'vendor'),
            request.args.get('code', ''),
        ))

    @app.route('/api/accounting/sage/audit-export', methods=['GET'])
    @login_required
    def api_acct_sage_audit_export():
        from accounting_waves_28 import sage_sync_audit_export
        return jsonify(sage_sync_audit_export(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/payroll/banner', methods=['GET'])
    @login_required
    def api_acct_sage_payroll_banner():
        from accounting_waves_28 import sage_payroll_banner_state
        return jsonify(sage_payroll_banner_state(db, models, _ledger_id()))

    @app.route('/api/accounting/compliance/filing-bundle/download', methods=['GET'])
    @login_required
    def api_acct_filing_download():
        from accounting_waves_28 import filing_bundle_download
        yr = request.args.get('tax_year', type=int) or date.today().year
        return jsonify(filing_bundle_download(db, models, _ledger_id(), yr, user_id=current_user.id))

    @app.route('/api/accounting/documents/<doc_type>/<int:doc_id>/line-tax', methods=['POST'])
    @login_required
    def api_acct_line_tax(doc_type, doc_id):
        from accounting_waves_28 import set_document_line_tax
        data = request.get_json(silent=True) or {}
        out = set_document_line_tax(db, models, _ledger_id(), doc_type, doc_id, data.get('lines') or [], user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/platform/ledger-by-company', methods=['GET'])
    @login_required
    def api_acct_ledger_by_co():
        from accounting_waves_28 import resolve_ledger_for_company
        return jsonify(resolve_ledger_for_company(db, models, request.args.get('company_code', '')))

    @app.route('/api/accounting/documents/open', methods=['GET'])
    @login_required
    def api_acct_open_docs_paged():
        from accounting_waves_28 import paginated_open_documents
        return jsonify(paginated_open_documents(
            db, models, _ledger_id(),
            request.args.get('type', 'ar'),
            request.args.get('offset', 0, type=int),
            request.args.get('limit', 50, type=int),
        ))

    @app.route('/api/accounting/cash/month-close-wizard', methods=['GET'])
    @login_required
    def api_acct_month_close_wizard():
        from accounting_waves_28 import month_close_wizard_state
        return jsonify(month_close_wizard_state(db, models, _ledger_id()))

    @app.route('/api/accounting/sage/read-only', methods=['GET', 'POST'])
    @login_required
    def api_acct_sage_read_only():
        from accounting_waves_28 import save_sage_read_only_mode
        if request.method == 'GET':
            from accounting_waves_24 import _ledger_settings
            ledger = models['AcctLedger'].query.get(_ledger_id())
            return jsonify({'enabled': _ledger_settings(ledger).get('sage_read_only_mode') == '1'})
        data = request.get_json(silent=True) or {}
        out = save_sage_read_only_mode(db, models, _ledger_id(), bool(data.get('enabled')), user_id=current_user.id)
        db.session.commit()
        return jsonify({'ok': True, **out})

    @app.route('/api/accounting/cron/sage-enterprise', methods=['POST'])
    def api_acct_cron_sage_enterprise():
