...
def document_sheet_editor_page():
    return render_template('office_editor.html', active_project=get_active_project(), editor_kind='sheet')


@app.route('/documents/word')
@login_required
def document_word_editor_page():
    return render_template('office_editor.html', active_project=get_active_project(), editor_kind='doc')


@app.route('/documents/sheet/legacy')
@login_required
def document_sheet_editor_legacy_page():
    return render_template('sheet_editor.html', active_project=get_active_project())


@app.route('/documents/word/legacy')
@login_required
def document_word_editor_legacy_page():
    return render_template('word_editor.html', active_project=get_active_project())


@app.route('/api/documents/office/create', methods=['POST'])
@login_required
def api_office_create_document():
    """Create a blank spreadsheet or document for LibreOffice Online editing."""
    from document_integration import guess_mime
    from document_persistence import ensure_document_schema, ensure_system_folders, resolve_my_files_folder
    from office_editor_service import editor_kind_to_ext, ensure_office_file_on_disk, office_enabled

    ensure_project_schema()
    ensure_document_schema(db.engine, db)

    body = request.get_json(silent=True) or {}
    kind = (request.form.get('kind') or body.get('kind') or 'sheet').strip()
    name = (request.form.get('name') or body.get('name') or '').strip()
    name = name or ('Untitled Spreadsheet' if kind == 'sheet' else 'Untitled Document')
    project_id = request.form.get('project_id', type=int) or body.get('project_id') or get_current_project_id()
    if not project_id:
    ...
    folder_id = request.form.get('folder_id', type=int) or body.get('folder_id')
    if not folder_id:
    ...
    doc = Document(
        ...
        mime_type=guess_mime(f'file.{ext}'),
        uploaded_by_id=current_user.id,
    ...
    ensure_office_file_on_disk(doc, upload_root=upload_root, User=User)
    db.session.commit()
    return jsonify({
        'ok': True,
        'id': doc.id,
...
@app.route('/api/documents/office/editor-session', methods=['GET'])
@login_required
def api_office_editor_session():
    """Return a Collabora/LibreOffice Online editor URL for a document."""
    from financial_security import require_financial_project_access
    ...
    token = issue_wopi_token(doc_id=doc.id, user_id=current_user.id, write=True)
    wopi_src = build_wopi_src(doc_id=doc.id, access_token=token)
    ext = extension_for_document(doc)
    try:
        editor_url = build_collabora_editor_url(wopi_src=wopi_src, ext=ext)
    except Exception as exc:
        return jsonify({'error': f'LibreOffice Online unavailable: {exc}'}), 503
...
@app.route('/wopi/files/<int:file_id>', methods=['GET', 'POST'])
def wopi_file(file_id):
    from office_wopi import handle_wopi_file
    upload_root = app.config.get('UPLOAD_FOLDER', 'uploads')
    return handle_wopi_file(file_id, Document=Document, Project=Project, User=User, db=db, upload_root=upload_root)


@app.route('/wopi/files/<int:file_id>/contents', methods=['GET', 'HEAD', 'POST', 'PUT'])
def wopi_file_contents(file_id):
    from office_wopi import handle_wopi_contents
    upload_root = app.config.get('UPLOAD_FOLDER', 'uploads')
    return handle_wopi_contents(file_id, Document=Document, Project=Project, User=User, db=db, upload_root=upload_root)
